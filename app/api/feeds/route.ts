...
export async function GET(){
  ...
  if(flightResult.status==='fulfilled'&&flightResult.value.ok){const data=await flightResult.value.json() as {states?:unknown[][]};flights=(data.states||[]).filter(s=>s[5]!=null&&s[6]!=null).map(s=>({icao:s[0],callsign:String(s[1]||'UNKNOWN').trim(),country:s[2],timePosition:s[3],lastContact:s[4],lon:s[5],lat:s[6],altitude:s[7],ground:s[8],velocity:s[9],heading:s[10],verticalRate:s[11],squawk:s[14],category:s[17]}));}else errors.push('OpenSky temporarily unavailable');
